...
namespace SmartASSWeb.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(IFirebaseService service, IUniqueCodeGenerator uniqueCodeGenerator)
        {
            _service = service;
        ...
        [HttpPost]
        [AllowAnonymous]
        public async Task<JsonResult> LoginFromFirebase(string uid)
        {
            if (string.IsNullOrEmpty(uid)) return Json("Bad");
            var user = await _service.Get<UserProfile>("user-profile", uid);
            ...
            identity.AddClaim(new Claim("package", user.Package));
            identity.AddClaim(new Claim("photoUrl", user.PhotoUrl));

            //if (string.IsNullOrEmpty(user.ProfileId))
            //{
            //    await _service.Update(uid, FirestoreTableStore.UserProfiles, "profileId", _uniqueCodeGenerator.Generate(6));
            //}
            //////////////////////////////////////////////////////
            ...
            var a = FormsAuthentication.Decrypt(encTicket);
